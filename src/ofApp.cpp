...
void ofApp::drawInformation() {
    ...
    if (uiInforOnOff) {
        ...
        if (informationStream.size() > 0) {
            information.drawString(informationStream[1], 10, 20 + 0 * 16);
            information.drawString(informationStream[5], 10, 20 + 2 * 16);
        }
        
        information.drawString(ofToString(pathMake.point3D.size()), 10, 20 + 1 * 16);
        ...
        for (int i = 0; i<vMeshPos1.size(); i++) {
            ofDrawLine(0, i * 3, 0 + vMeshPos1[i].x, i * 3);
        }
        
        ofTranslate(80, 0);
        for (int i = 0; i<vMeshPos1.size(); i++) {
            ofDrawLine(0, i * 3, 0 + vMeshPos1[i].y, i * 3);
        }
        
        ofTranslate(80, 0);
        for (int i = 0; i<vMeshPos1.size(); i++) {
            ofDrawLine(0, i * 3, 0 + vMeshPos2[i].x, i * 3);
        }
        
        ofTranslate(80, 0);
        for (int i = 0; i<vMeshPos1.size(); i++) {
            ofDrawLine(0, i * 3, 0 + vMeshPos2[i].y, i * 3);
        }
        
        ofTranslate(ofGetWidth() * 0.5, 0);
        ofTranslate(0, 0);
        for (int i = 0; i<vMeshDir1.size(); i++) {
            ofDrawLine(0, i * 3, 0 + vMeshDir1[i].x, i * 3);
        }
        
        ofTranslate(80, 0);
        for (int i = 0; i<vMeshDir1.size(); i++) {
            ofDrawLine(0, i * 3, 0 + vMeshDir1[i].y, i * 3);
        }
        
        ofTranslate(80, 0);
        for (int i = 0; i<vMeshDir1.size(); i++) {
            ofDrawLine(0, i * 3, 0 + vMeshDir2[i].x, i * 3);
        }
...
void ofApp::keyReleased(int key) {
    ...
    switch (key) {
        case 'c':
            _pix.allocate(ofGetWidth(), ofGetHeight(), OF_IMAGE_COLOR_ALPHA);
            _pix.grabScreen(0, 0, ofGetWidth(), ofGetHeight());
            _file = "../../../" + ofGetTimestampString() + " Capture.jpg";
            _pix.save(_file, OF_IMAGE_QUALITY_BEST);
            break;
